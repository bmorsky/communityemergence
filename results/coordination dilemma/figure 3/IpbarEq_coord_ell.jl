# Figure 3b: total cooperation Ip̄ at equilibrium vs ℓ
...
for m = 1:1:1000
    ...
    I = K*(R .- 1)./(R .+ ι./φ)
    I[findmax(p̄)[2]] = K
    global higheq = vcat(higheq, [ℓ p̄[1]*I[1] 0])
    if length(eqp) > 1
            if routhHurwitz(ℓ,eqp[2])
                    global mideq = vcat(mideq, [ℓ p̄[2]*I[2] 1])
            else
                    global mideq = vcat(mideq, [ℓ p̄[2]*I[2] 2])
            end
            if routhHurwitz(ℓ,eqp[3])
                    global loweq = vcat(loweq, [ℓ p̄[3]*I[3] 3])
            else
                    global loweq = vcat(loweq, [ℓ p̄[3]*I[3] 4])
            end
    end
end
...
save_plot(p,filename="IpbarEq_coord_ell_iota1omega02phi1.png",base_height = 3.5,base_width = 3.5)
"""
