...
for m = 1:1:1000
    ...
    I = K*(R .- 1)./(R .+ ι./φ)
    I[findmax(p̄)[2]] = K
    global higheq = vcat(higheq, [ω I[1] 0])
    if length(eqp) > 1
            if routhHurwitz(ω,eqp[2])
                    global mideq = vcat(mideq, [ω I[2] 1])
            else
                    global mideq = vcat(mideq, [ω I[2] 2])
            end
            if routhHurwitz(ω,eqp[3])
                    global loweq = vcat(loweq, [ω I[3] 3])
            else
                    global loweq = vcat(loweq, [ω I[3] 4])
            end
    end
end
...
save_plot(p,filename="IEq_coord_omega_iota1ell1phi1.png",base_height = 3.5,base_width = 3.5)
"""
