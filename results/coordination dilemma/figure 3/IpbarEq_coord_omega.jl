# Figure 3d: total cooperation Ip̄ at equilibrium vs ω
...
for m = 1:1:1000
    ...
    I = K*(R .- 1)./(R .+ ι./φ)
    I[findmax(p̄)[2]] = K
    global higheq = vcat(higheq, [ω p̄[1]*I[1] 0])
    if length(eqp) > 1
            if routhHurwitz(ω,eqp[2])
                    global mideq = vcat(mideq, [ω p̄[2]*I[2] 1])
            else
                    global mideq = vcat(mideq, [ω p̄[2]*I[2] 2])
            end
            if routhHurwitz(ω,eqp[3])
                    global loweq = vcat(loweq, [ω p̄[3]*I[3] 3])
            else
                    global loweq = vcat(loweq, [ω p̄[3]*I[3] 4])
            end
    end
end
...
p <- ggplot() +
        ...
        geom_text(data=bifpoint,aes(x=V1, y=V2),label = expression(paste(omega,"*")),hjust=-0.3,vjust=0.07) +
...
save_plot(p,filename="IpbarEq_coord_omega_iota1ell1phi1.png",base_height = 3.5,base_width = 3.5)
"""
