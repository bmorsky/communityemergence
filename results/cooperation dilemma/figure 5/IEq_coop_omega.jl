    ...
    if length(eqp) == 1
        if counter
            if routhHurwitz(ω,eqp[1])
                global loweq = vcat(loweq, [ω I[1] 1])
            else
                global loweq = vcat(loweq, [ω I[1] 2])
            end
        else
            if routhHurwitz(ω,eqp[1])
                    global higheq = vcat(higheq, [ω I[1] 5])
            else
                    global higheq = vcat(higheq, [ω I[1] 6])
            end
        end
    else length(eqp) > 1
        if routhHurwitz(ω,eqp[1])
        ...
        if routhHurwitz(ω,eqp[2])
                global mideq = vcat(mideq, [ω I[2] 3])
        else
                global mideq = vcat(mideq, [ω I[2] 4])
        end
        if routhHurwitz(ω,eqp[3])
                global loweq = vcat(loweq, [ω I[3] 1])
        else
                global loweq = vcat(loweq, [ω I[3] 2])
        end
...
save_plot(p,filename="IEq_coop_omega_iota1ell1phi1.png",base_height = 3.5,base_width = 3.5)
"""
