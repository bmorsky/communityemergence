# Figure 5d: total cooperation Ip̄ at equilibrium vs ω
    ...
    if length(eqp) == 1
        if counter
            if routhHurwitz(ω,eqp[1])
                global loweq = vcat(loweq, [ω p̄[1]*I[1] 1])
            else
                global loweq = vcat(loweq, [ω p̄[1]*I[1] 2])
            end
        else
            if routhHurwitz(ω,eqp[1])
                    global higheq = vcat(higheq, [ω p̄[1]*I[1] 5])
            else
                    global higheq = vcat(higheq, [ω p̄[1]*I[1] 6])
            end
        end
    else length(eqp) > 1
        if routhHurwitz(ω,eqp[1])
        ...
        if routhHurwitz(ω,eqp[2])
                global mideq = vcat(mideq, [ω p̄[2]*I[2] 3])
        else
                global mideq = vcat(mideq, [ω p̄[2]*I[2] 4])
        end
        if routhHurwitz(ω,eqp[3])
                global loweq = vcat(loweq, [ω p̄[3]*I[3] 1])
        else
                global loweq = vcat(loweq, [ω p̄[3]*I[3] 2])
        end
...
bifpoints = [loweq[end,1:2]'; higheq[1,1:2]']
...
p <- ggplot() +
        ...
        geom_text(data=bifpoints,aes(x=V1, y=V2),label = c(expression(paste(omega[2],"*")),expression(paste(omega[1],"*"))),hjust=c(-0.1,1),vjust=0.07) +
...
save_plot(p,filename="IpbarEq_coop_omega_iota1ell1phi1.png",base_height = 3.5,base_width = 3.5)
"""
