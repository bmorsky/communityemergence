# Figure 6a: equilibrium community size I vs ι for the cooperation dilemma
...
function crashTest(ι)
    ...
    p = find_zeros(p -> ω*solỹ(p)*soly(p)^2 + ω*(q̂-q)*soly(p)^2 - (ℓ+ω)*solỹ(p)*soly(p) - ((ℓ+ω)*(q̂-q) + ι)*soly(p) + ℓ*solỹ(p) + ℓ*(q̂-q), 0, q, no_pts=300)
    p = p[0 .<= p .<= 1]
    y = soly(p)
...
for m = 1:1:1000
    ...
    if ~isempty(eqp)
        if routhHurwitz(ι,eqp[1])
            global higheq = vcat(higheq, [ι I[1] 1])
        else
            global higheq = vcat(higheq, [ι I[1] 2])
        end
        if length(eqp) == 2
            if routhHurwitz(ι,eqp[2])
                global mideq = vcat(mideq, [ι I[2] 3])
            else
                global mideq = vcat(mideq, [ι I[2] 4])
            end
        end
        if length(eqp) == 3
            if routhHurwitz(ι,eqp[3])
                global loweq = vcat(loweq, [ι I[3] 5])
            else
                global loweq = vcat(loweq, [ι I[3] 6])
            end
        end
    end
end
...
bifpoints = vcat(crasheq[end,1:2]',[crasheq[end,1] higheq[length(crasheq[:,1])-Int(0.5*1000*higheq[1,1]),2]])
...
save_plot(p,filename="crash_coop_iota_ell1omega1phi1.png",base_height = 3.5,base_width = 3.5)
"""
