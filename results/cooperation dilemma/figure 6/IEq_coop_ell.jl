...
for m = 1:1:5000
    ...
    if length(eqp) == 1
        if counter
            ...
            if routhHurwitz(ℓ,eqp[1])
                    global loweq = vcat(loweq, [ℓ I[1] 5])
            else
                    global loweq = vcat(loweq, [ℓ I[1] 6])
            end
        end
    else length(eqp) > 1
        if routhHurwitz(ℓ,eqp[1])
...
save_plot(p,filename="IEq_coop_ell_iota1omega1phi1.png",base_height = 3.5,base_width = 3.5)
"""
