# Appendix C Figure 12a: equilibrium community size I vs ι for the coordination dilemma
...
crashy = Array{Float64}(undef, 0, 4)

function routhHurwitz(ι,p)
...
for m = 1:1:1000
    ι = 5e-5*m
        ...
        # check if the conditions are met
        if crashTest(ι)
            global crasheq = vcat(crasheq, [ι 0 0])
        end
        ...
        global higheq = vcat(higheq, [ι K 1])
        if ~isempty(eqp)
            if length(eqp) == 1
                if routhHurwitz(ι,eqp[1])
                    global mideq = vcat(mideq, [ι I[1] 3])
                else
                    global mideq = vcat(mideq, [ι I[1] 4])
                end
            end
            if length(eqp) == 2
                ...
                if routhHurwitz(ι,eqp[2])
                    global loweq = vcat(loweq, [ι I[2] 5])
                else
                    global loweq = vcat(loweq, [ι I[2] 6])
                end
            end
        end
    end
...
bifpoints = vcat(crasheq[end,1:2]',[crasheq[end,1] 100])
...
colscolour = c("-1"="black","0"="black","1"="black","2"="magenta","3"="black","4"="magenta","5"="black","6"="magenta")
...
p <- ggplot() +
        ...
        geom_text(data=bifpoints,aes(x=V1, y=V2),label = c(expression(paste(iota,"*")),expression(paste(iota,"*"))),hjust=-0.7,vjust=9) +
        scale_y_continuous(expand = c(0, 0),breaks=c(0,25,50,75,100),labels=c("0","K/4","K/2","3K/4","K"), lim = c(-1.5,101)) +
        scale_x_continuous(expand = c(0, 0),breaks=c(0,0.0125,0.025,0.0375,0.05),labels=c(0,0.0125,0.025,0.0375,0.05),lim = c(0,0.0512)) +
...
save_plot(p,filename="crash_coord_iota_ell1omega1phi1.png",base_height = 3.5,base_width = 3.5)
"""
