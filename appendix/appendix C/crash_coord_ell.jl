# Appendix C Figure 12b: equilibrium community size I vs ℓ for the coordination dilemma
...
for m = 1:1:1000
    ...
    global higheq = vcat(higheq, [ℓ K 1])
    if ~isempty(eqp)
        if length(eqp) == 1
            if routhHurwitz(ℓ,eqp[1])
                global mideq = vcat(mideq, [ℓ I[1] 3])
            else
                global mideq = vcat(mideq, [ℓ I[1] 4])
            end
        end
        if length(eqp) == 2
            ...
            if routhHurwitz(ℓ,eqp[2])
                global loweq = vcat(loweq, [ℓ I[2] 5])
            else
                global loweq = vcat(loweq, [ℓ I[2] 6])
            end
        end
    end
end
...
p <- ggplot() +
        ...
        geom_text(data=bifpoints,aes(x=V1, y=V2),label = expression(paste('\u2113',"*")),hjust=1.3,vjust=11) +
...
save_plot(p,filename="crash_coord_ell_iota01omega1phi1.png",base_height = 3.5,base_width = 3.5)
"""
